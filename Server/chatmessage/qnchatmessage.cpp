...
//    te_font.setLetterSpacing(QFont::AbsoluteSpacing, 0);             //设置字间距为3像素 //设置字间距像素值
    ...
    m_rightPixmap = QPixmap("images/nzk.ico");
...
//        painter.drawRoundedRect(m_iconLeftRect,m_iconLeftRect.width(),m_iconLeftRect.height());
        ...
        //头像
        painter.drawRoundedRect(m_iconRightRect,m_iconRightRect.width(),m_iconRightRect.height());//圆形头像
...
QPixmap QNChatMessage::getRightPixmap() const
{
    return m_rightPixmap;
}

void QNChatMessage::setRightPixmap(const QPixmap &rightPixmap)
{
    m_rightPixmap = rightPixmap;
}

QPixmap QNChatMessage::getLeftPixmap() const
{
    return m_leftPixmap;
}

void QNChatMessage::setLeftPixmap(const QPixmap &leftPixmap)
{
    m_leftPixmap = leftPixmap;
}
